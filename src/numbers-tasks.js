...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const prodNumbers = x1 * x2 + y1 * y2;
  const length = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(prodNumbers / length);
}
...
function roundToPowerOfTen(num, pow) {
  const degree = 10 ** pow;
  const number = num / degree;
  return pow === 0 ? num : Math.round(number) * degree;
}
...
function toNumber(value, def) {
  ...
  return Number.isNaN(numb) ? def : numb;
}
